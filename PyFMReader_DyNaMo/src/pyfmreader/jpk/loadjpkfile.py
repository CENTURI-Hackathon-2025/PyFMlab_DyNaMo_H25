...
def loadJPKfile(filepath, UFF, filesuffix):
    """
    Function used to load the metadata of a JPK file from the zip file stored in cache .
    ...
    headercontents = UFF.zipbuffer.get_file('header.properties')
    ...
    sharedheadercontents = UFF.zipbuffer.get_file('shared-data/header.properties')
    ...
    paths = [name for name in UFF.zipbuffer.namelist() if "segments" in name]
    ...
    for segment_group in groupded_paths:
        ...
        for path in segment_group:
            ...
            if data_type == 'segment-header':
                metadatacontents = UFF.zipbuffer.get_file(path)
